...
func (c *API) FetchServices(
	workspacesID zoho.Parameter,
	serviceID zoho.Parameter,
	staffID zoho.Parameter,
) (data ServiceResponse, err error) {
	...
	if workspacesID == "" {
		return ServiceResponse{}, fmt.Errorf(
			"Failed to execute FetchServices due to non-availability of workspace_id",
		)
	}
